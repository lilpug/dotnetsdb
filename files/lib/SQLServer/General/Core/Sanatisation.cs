    ...
    public partial class SqlServerCore
    {
        ...
        protected void SanitisationProcess(ref SqlCommand command)
        {
            ...
            for (int qc = 0; qc < theQueries.Count; qc++)
            {
                ...
                {
                    SanitiseItems(string.Format("{0}_{1}_0_", existDefinition, realQueryCount.ToString()), ref command, theQueries2[index].existRealTableValue);
                }

                if (theQueries2[index].getFieldsRealTableValue != null && theQueries2[index].getFieldsRealTableValue.Length > 0)
                {
                    SanitiseItems(string.Format("{0}_{1}_0_", fieldsDefinition, realQueryCount.ToString()), ref command, theQueries2[index].getFieldsRealTableValue);
                }
