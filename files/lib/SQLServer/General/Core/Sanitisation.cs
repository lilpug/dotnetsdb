...
namespace DotNetSDB
{
    public partial class SqlServerCore
    {
        ...
        protected virtual void SanitiseItems(string definition, ref SqlCommand command, params object[] items)
        {
            ...
            try
            {
                foreach (object data in items)
                {
                    ...
                    if (data != null)
                    {
                        using (DatabaseTypeConvertor convertor = new DatabaseTypeConvertor())
                        {
        ...
        protected virtual void ExtraSanitisationProcessing(Query current, ref SqlCommand command, int queryCounter)
        {   
